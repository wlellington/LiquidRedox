...
class DynamicSequences(Module):
    ...
    def record_sequence(self, keyboard, slot, *args, **kwargs):
        self.current_ind = slot
        self.current_slot = self.sequences[slot]
        self.status = SequenceStatus.RECORDING
        self.start_time = ticks_ms()
        self.current_slot.sequence_data = [SequenceFrame(set(), 0)]
        self.index = 0

    # Public version of play
    def play_sequence(self, keyboard, slot, *args, **kwargs):
        self.current_ind = slot
        self.current_slot = self.sequences[slot]
        self.stop_sequence(keyboard, slot)
        self.status = SequenceStatus.PLAYING
        self.start_time = ticks_ms()
        self.index = 0
        self.current_repetition = 0

    # Public version of stop
    def stop_sequence(self, keyboard, slot, *args, **kwargs):
        self.current_ind = slot
        self.current_slot = self.sequences[slot]
    ...
    def stop_recording(self):
        ...
        #print("Recorded: ", self.current_slot.sequence_data)
        self.status = SequenceStatus.STOPPED
